#ifndef __VERSION_HEADER__
#define __VERSION_HEADER__

namespace Version{
	
	//Date Version Types
	#define DEFINED_VER_DATE "20"
	#define DEFINED_VER_MONTH "08"
	#define DEFINED_VER_YEAR "2015"
	#define DEFINED_VER_UBUNTU_VERSION_STYLE  "15.08"
	
	//Software Status
	#define DEFINED_VER_STATUS  "Beta"
	#define DEFINED_VER_STATUS_SHORT  "b"
	
	//Standard Version Type
	#define DEFINED_VER_MAJOR  4
	#define DEFINED_VER_MINOR  0
	#define DEFINED_VER_BUILD  0
	#define DEFINED_VER_REVISION  1279
	
	//Miscellaneous Version Types
	#define DEFINED_VER_BUILDS_COUNT  129
	#define DEFINED_VER_RC_FILEVERSION 4,0,0,1279
	#define DEFINED_VER_RC_FILEVERSION_STRING "4, 0, 0, 1279\0"
	#define DEFINED_VER_FULLVERSION_STRING  "4.0.0.1279"
	
	//These values are to keep track of your versioning state, don't modify them.
	#define DEFINED_VER_BUILD_HISTORY  10
	

}
#endif //__VERSION_HEADER__
